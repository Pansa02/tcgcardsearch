...
&categoryName=CCG%20Individual%20Cards";
    ...
    if (maxPrice !== "") {
      url =
        url +
        "&itemFilter(" +
        String(itemFilterGroup) +
        ").name=MaxPrice&itemFilter(0).value=" +
        maxPrice +
        "&itemFilter(" +
        String(itemFilterGroup) +
    ...
    if (auctionEndTime !== "//") {
      ...
      const apiUrlTimeLeft =
        "&itemFilter(" +
        String(itemFilterGroup) +
        ").name=EndTimeTo&itemFilter(" +
        String(itemFilterGroup) +
        ").value=" +
        adjustedTimeLeft.toISOString();
    ...
    cardResponse =
      cardResponse + "-----------" + cardName.toUpperCase() + "-----------\n\n";
    if (
      apiResult.findItemsAdvancedResponse[0].searchResult[0]["@count"] !== "0"
    ) {
      for (let x = 0; x < itemsList.length; x++) {
        if (immediateBuyout === true) {
          if (
            itemsList[x]["listingInfo"][0].buyItNowAvailable[0] !== "true" &&
            itemsList[x]["listingInfo"][0].listingType[0] !== "FixedPrice"
          ) {
            continue;
          }
        }
        ...
        const condition = setCondition(
          () => itemsList[x]["condition"][0].conditionDisplayName[0]
        );
        const price =
          itemsList[x]["sellingStatus"][0].convertedCurrentPrice[0][
            "__value__"
          ];
        const shipping =
          itemsList[x]["shippingInfo"][0].shippingServiceCost[0]["__value__"] +
          " " +
          itemsList[x]["shippingInfo"][0].shippingServiceCost[0]["@currencyId"];
